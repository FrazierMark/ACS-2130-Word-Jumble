...
def word_to_dict(word_to_descramble):
  ...
  # create the dict letter to quantity: {'t': 3, 'r': 1}
...
def find_possible_len_words(letter_count_dict, scrambled_word_length):
    ...
    for word in dictionary_list:
        if len(word) == scrambled_word_length:  # Compare to actual word length
            possible_answer = dict(word_to_dict(word))  # Letter frequency of dict word
...
num_words = ask_for_number_of_words()  # Ask for the number of words to descramble
all_possible_words = []  # List to store possible words for all scrambled inputs

for _ in range(num_words):
    word_to_descramble = ask_for_jumble()  # Get each scrambled word
    letter_count_dict = dict(word_to_dict(word_to_descramble))  # Letter count of scrambled word
    possible_words = find_possible_len_words(letter_count_dict, len(word_to_descramble))  # Find possible words
    all_possible_words.append(possible_words)  # Store the possible words for this input

print("Possible words for each input:", all_possible_words)  # Print all possible words
